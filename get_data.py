...
def get_data(username, password):
    ...
    if os.path.exists(cached_records_path):
        ...
        os.makedirs(os.path.dirname(cached_records_path), exist_ok=True)
        cached_records = None
        last_sync = None
    ...
    print(f'Retrieved {year_}-{month_}, page {1} of {n_pages} ...')
    if n == 0:
    ...
    for p in range(1, n_pages):
        ...
        time.sleep(np.random.uniform(low=0.7, high=1.3))
        print(f'Retrieved {year_}-{month_}, page {p + 1} of {n_pages} ...')
        record['date_and_time'] = pd.to_datetime(record['date_and_time'], unit='ms')
        records.append(record)
    while 1:
